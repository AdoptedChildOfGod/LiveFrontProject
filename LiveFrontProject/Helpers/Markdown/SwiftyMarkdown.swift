...
@objc open class BasicStyles: NSObject, FontProperties {
    public var fontName: String? = FontNames.regular.rawValue
    public var color = UIColor.text
...
@objc open class LinkStyles: BasicStyles {
    ...
    public lazy var underlineColor = UIColor.systemBlue
}
...
@objc open class SwiftyMarkdown: NSObject {
    public static var frontMatterRules = [FrontMatterRule(openTag: "---", closeTag: "---", keyValueSeparator: ":")]

    public static var lineRules = [LineRule(token: "=", type: MarkdownLineStyle.previousH1, removeFrom: .entireLine, changeAppliesTo: .previous),
    ...
    public static var characterRules = [CharacterRule(primaryTag: CharacterRuleTag(tag: "![", type: .open), otherTags: [CharacterRuleTag(tag: "]", type: .close),
                                                                                                                        CharacterRuleTag(tag: "[", type: .metadataOpen),
                                                                                                                        CharacterRuleTag(tag: "]", type: .metadataClose)], styles: [1: CharacterStyle.image], metadataLookup: true, definesBoundary: true),
    CharacterRule(primaryTag: CharacterRuleTag(tag: "![", type: .open), otherTags: [CharacterRuleTag(tag: "]", type: .close),
                                                                                    CharacterRuleTag(tag: "(", type: .metadataOpen),
                                                                                    CharacterRuleTag(tag: ")", type: .metadataClose)], styles: [1: CharacterStyle.image], metadataLookup: false, definesBoundary: true),
    CharacterRule(primaryTag: CharacterRuleTag(tag: "[", type: .open), otherTags: [CharacterRuleTag(tag: "]", type: .close),
                                                                                   CharacterRuleTag(tag: "[", type: .metadataOpen),
                                                                                   CharacterRuleTag(tag: "]", type: .metadataClose)], styles: [1: CharacterStyle.link], metadataLookup: true, definesBoundary: true),
    CharacterRule(primaryTag: CharacterRuleTag(tag: "[", type: .open), otherTags: [CharacterRuleTag(tag: "]", type: .close),
                                                                                   CharacterRuleTag(tag: "(", type: .metadataOpen),
                                                                                   CharacterRuleTag(tag: ")", type: .metadataClose)], styles: [1: CharacterStyle.link], metadataLookup: false, definesBoundary: true),
    ...
    public var frontMatterAttributes: [String: String] {
        return lineProcessor.frontMatterAttributes
    }
     ...
     */
    public init(string: String, maxImageWidth: CGFloat? = nil) {
        self.string = string
...
extension SwiftyMarkdown {
    func attributedStringFor(tokens: [Token], in line: SwiftyLine) -> NSAttributedString {
        ...
        case .unorderedList, .unorderedListIndentFirstOrder, .unorderedListIndentSecondOrder, .orderedList, .orderedListIndentFirstOrder, .orderedListIndentSecondOrder:
            ...
            var indent = ""
            if let lineStyle = line.lineStyle as? MarkdownLineStyle {
                switch lineStyle {
            ...
            if let imgIdx = styles.firstIndex(of: .image), imgIdx < token.metadataStrings.count {
                ...
                if let url = URL(string: token.metadataStrings[imgIdx]),
                   let data = try? Data(contentsOf: url) {
                    ...
                    let width = maxImageWidth ?? 100.0
                    let height = ((image?.size.height ?? 0) / (image?.size.width ?? 1)) * width
                    image1Attachment.bounds = CGRect(x: 0, y: 0, width: width, height: height)
                }
