...
class DetailViewController: BaseViewController {
    ...
    override func loadView() {
        ...
        contentLabel.setVisible(if: rule?.subsections?.isEmpty ?? true)
    ...
    private lazy var contentLabel = MarkdownView()
    /// The css styling for the markdown
    private let css = "body { color:#FFF4C7; }"
    ...
    override func addAllSubviews() {
        view.addSubviews(contentLabel, subsectionsTableView)
    }

    /// Set up the constraints and lay out all the elements in the view
    override func setUpConstraints() {
        // The label with the content of the rule, if applicable
        contentLabel.anchorFill(view, safeArea: true)
